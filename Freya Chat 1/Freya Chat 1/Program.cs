...
namespace Freya_Chat_1
{
    class Program
    {
            ...
            private void Receive()
            {
                _socket.BeginReceiveFrom(state.buffer, 0, bufSize, SocketFlags.None, ref epFrom, recv = (ar) =>
                {
                    try
                    {
                        ...
                        Console.WriteLine("Karşı taraf: {2}", epFrom.ToString(), bytes, Encoding.ASCII.GetString(so.buffer, 0, bytes));
                    }
                    catch { }
        ...
        static void Main(string[] args)
        {
            ...
            //s.Send("Bu mesaj serverdan gelmiştir.");
